...
/// Stored as a normalised pair (alphabetically lower name first).
/// </summary>
public class FriendStore
{
    private readonly RendezvousDb _db;

    public FriendStore(RendezvousDb db) => _db = db;
    ...
    public List<string> GetFriends(string username)
    {
        ...
        using var conn = _db.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT user2 FROM friends WHERE user1 = $u UNION SELECT user1 FROM friends WHERE user2 = $u";
